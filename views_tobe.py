...
def course(request, requested_department):
    ...
    if requested_department in department:
        ...
        for each in matching_course.all():
            tutorials = Tutorial.objects.filter(
                course_name__course_name=each.course_name).earliest('pub_date')
            # print(tutorials)
            tutorial_urls[each.id]=tutorials.course_name
            print("Key ",each.id, "Value ", tutorials.course_name)
    ...
    return HttpResponse(f"{requested_department} does not correspond to anything :(")

def tutorial(request, requested_department, course_name, requested_url):
    department = [d.department_name for d in Department.objects.all()]
    print(department)
    # print(int(requested_department.id))
    if requested_department in department:
        matching_course = Course.objects.filter(
            department_name__department_name=requested_department)
        for m in matching_course:
            print(m)
        tutorial_urls={} # all tutorials found in requested department
        for each in matching_course.all():
            tutorials = Tutorial.objects.filter(
                course_name__course_name=each.course_name).earliest('pub_date')
            print(each.course_name," + ",tutorials.id)
            print(each.id, " *** ",tutorials.id)
            tutorial_urls[each]=tutorials.course_name
        for i in tutorial_urls:
            print(i.id)
    ...
    """
    return HttpResponse(f"Tutorial {requested_url} does not existed or it must be removed :(")
